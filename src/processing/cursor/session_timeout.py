...
class CursorSessionTimeoutManager:
    ...
    async def cleanup_stale_sessions(self):
        ...
        try:
            ...
            # Query database for old active Cursor sessions
            ...
            logger.info(f"Found {len(stale_sessions)} stale Cursor sessions to timeout")
            ...
            for external_session_id, started_at_str in stale_sessions:
                try:
                    ...
                    logger.error(f"Failed to timeout Cursor session {external_session_id}: {e}", exc_info=True)
